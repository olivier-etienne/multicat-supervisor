 ...
 * $Id: util.c 60 2014-09-18 16:37:50Z massiot $
 *
...
#ifdef DEBUG_SOCKET
	Logs(LOG_DEBUG,__FILE__,__LINE__,"socket definition:", &bind_addr, &connect_addr );	
#endif
...
void dump_eit_section(struct EitInfoSection * section ) {
	...
	printf("SHORT_EVENT_DESCRIPTOR - event_name : %s\n",section->short_event_desc.event_name);
	printf("SHORT_EVENT_DESCRIPTOR - event_text : %s\n",section->short_event_desc.event_text);
	printf("SHORT_EVENT_DESCRIPTOR - event_lang : %s\n",section->short_event_desc.event_lang);

	printf("EXTENDED_EVENT_DESCRIPTOR - text : %s\n",section->ext_event_desc.text);
	printf("EXTENDED_EVENT_DESCRIPTOR - lang : %s\n",section->ext_event_desc.lang);

	for(i=0;i<EXTEVENTDESC_ITEM_SIZE;i++) {
		printf("EXTENDED_EVENT_DESCRIPTOR - %d text : %s\n",i+1,section->ext_event_desc.items[i].text);
		printf("EXTENDED_EVENT_DESCRIPTOR - %d desc : %s\n",i+1,section->ext_event_desc.items[i].desc);
	}

	for (i=0;i<3;i++) {
		printf("COMPONENT_DESCRIPTOR [%d] - stream_content : %0x\n",i,section->component_desc[i].stream_content);
		printf("COMPONENT_DESCRIPTOR [%d] - component_type : %0x\n",i,section->component_desc[i].component_type);
		printf("COMPONENT_DESCRIPTOR [%d] - set_component_tag : %d\n",i,section->component_desc[i].set_component_tag);
		printf("COMPONENT_DESCRIPTOR [%d] - text : %s\n",i,section->component_desc[i].text);
		printf("COMPONENT_DESCRIPTOR [%d] - lang : %s\n",i,section->component_desc[i].lang);
	}
	
	printf("PARENTAL_RATING_DESCRIPTOR - country_code : %s\n",section->parent_rating_desc.country_code);
	printf("PARENTAL_RATING_DESCRIPTOR - age : %d\n",section->parent_rating_desc.age);

	for (i=0;i<CONTENT_DESC_SIZE;i++) {
		printf("CONTENT_DESCRIPTOR [%d] - level 1 : %d\n",i,section->content_desc[i].level_1);
		printf("CONTENT_DESCRIPTOR [%d] - level 2 : %d\n",i,section->content_desc[i].level_2);
		printf("CONTENT_DESCRIPTOR [%d] - user : %d\n",i,section->content_desc[i].user);
	}
	
	for(i=0;i<CA_SYSTEM_ID_SIZE;i++) {
		printf("CA IDENTIFER DESCRIPTOR [%d] : 0x%0x\n",i,section->ca_identifier_desc.CASystemId[i]);
	}

	printf("SHORT_SMOOTHING_BUFFER_DESCRIPTOR : %d\n",section->short_smoothing_Buffer_desc.sbLeakRate);	
...
void dump_eit_info(struct EitInfo * str_eit_info ) {
	...
	printf("--------------------------------------------------------------------------------\n");
	printf("initialized : %d\n",str_eit_info->initialized);
	printf("Updated : %d\n",str_eit_info->updated);
	printf("tsid : %d\n",str_eit_info->tsid);
	printf("sid : %d\n",str_eit_info->sid);
	printf("onid : %d\n",str_eit_info->onid);
	...
	printf("------------------- SECTION 0----------------\n");
	dump_eit_section(&str_eit_info->section0);

	printf("------------------- SECTION 1----------------\n");
	dump_eit_section(&str_eit_info->section1);
...
char * getValueFromIni(char * tag,char *prefix,int index)
{
	...
	sprintf(param,"%s_%d",prefix,index);
	char *value = rechercherValeur(tag,param);
...
/*****************************************************************************
 * load eit info from ini file
 *****************************************************************************/
void load_eit(struct EitInfo *str_eit_info)
{
	int i;
	char extended_event_descriptor_item_text[10];
	char extended_event_descriptor_item_desc[10];
	
	memset(str_eit_info,0,sizeof(struct EitInfo));
	
	str_eit_info->tsid = (uint16_t)convertStrToLong(rechercherValeur("EIT_MAIN_DESCRIPTOR","tsid"));
	str_eit_info->sid = (uint16_t)convertStrToLong(rechercherValeur("EIT_MAIN_DESCRIPTOR","sid"));
	str_eit_info->onid = (uint16_t)convertStrToLong(rechercherValeur("EIT_MAIN_DESCRIPTOR","onid"));
	
	strncpy(str_eit_info->section0.short_event_desc.event_name,rechercherValeur("SHORT_EVENT_DESCRIPTOR","event_name"),SHORTEVENTDESC_NAME_LENGTH);
	strncpy(str_eit_info->section0.short_event_desc.event_text,rechercherValeur("SHORT_EVENT_DESCRIPTOR","event_text"),SHORTEVENTDESC_TEXT_LENGTH);
	strncpy(str_eit_info->section0.short_event_desc.event_lang,rechercherValeur("SHORT_EVENT_DESCRIPTOR","event_lang"),SHORTEVENTDESC_LANG_LENGTH);

	strncpy(str_eit_info->section1.short_event_desc.event_name,rechercherValeur("SHORT_EVENT_FOLLOWING_DESCRIPTOR","event_name"),SHORTEVENTDESC_NAME_LENGTH);
	strncpy(str_eit_info->section1.short_event_desc.event_text,rechercherValeur("SHORT_EVENT_FOLLOWING_DESCRIPTOR","event_text"),SHORTEVENTDESC_TEXT_LENGTH);
	strncpy(str_eit_info->section1.short_event_desc.event_lang,rechercherValeur("SHORT_EVENT_FOLLOWING_DESCRIPTOR","event_lang"),SHORTEVENTDESC_LANG_LENGTH);

	strncpy(str_eit_info->section0.ext_event_desc.text,rechercherValeur("EXTENDED_EVENT_DESCRIPTOR","text"),EXTEVENTDESC_TEXT_LENGTH);
	strncpy(str_eit_info->section0.ext_event_desc.lang,rechercherValeur("EXTENDED_EVENT_DESCRIPTOR","lang"),EXTEVENTDESC_LANG_LENGTH);

	strncpy(str_eit_info->section1.ext_event_desc.text,rechercherValeur("EXTENDED_EVENT_DESCRIPTOR","text"),EXTEVENTDESC_TEXT_LENGTH);
	strncpy(str_eit_info->section1.ext_event_desc.lang,rechercherValeur("EXTENDED_EVENT_DESCRIPTOR","lang"),EXTEVENTDESC_LANG_LENGTH);

	for(i=0;i<EXTEVENTDESC_ITEM_SIZE;i++) {
		sprintf(extended_event_descriptor_item_text,"text_%d",i);
		sprintf(extended_event_descriptor_item_desc,"desc_%d",i);
		char *text = rechercherValeur("EXTENDED_EVENT_DESCRIPTOR",extended_event_descriptor_item_text);
		char *desc = rechercherValeur("EXTENDED_EVENT_DESCRIPTOR",extended_event_descriptor_item_desc);
		if ( text != NULL && desc != NULL ) {
			strncpy(str_eit_info->section0.ext_event_desc.items[i].text,text,TEXTANDDESC_TEXT_LENGTH);
			strncpy(str_eit_info->section0.ext_event_desc.items[i].desc,desc,TEXTANDDESC_DESC_LENGTH);
	...
	for(i=0;i<COMPONENTDESC_SIZE;i++) {
		char *stream_content = getValueFromIni("COMPONENT_DESCRIPTOR","stream_content",i+1);
		char *component_type = getValueFromIni("COMPONENT_DESCRIPTOR","component_type",i+1);
		char *set_component_tag = getValueFromIni("COMPONENT_DESCRIPTOR","set_component_tag",i+1);
		char *text = getValueFromIni("COMPONENT_DESCRIPTOR","text",i+1);
		char *lang = getValueFromIni("COMPONENT_DESCRIPTOR","lang",i+1);
		
		if ( *stream_content != '\0' && *component_type  != '\0' &&
				*set_component_tag != '\0' &&
				*text != '\0' &&
				*lang != '\0') {
				
			str_eit_info->section0.component_desc[i].stream_content = convertStrToInt(stream_content);
			str_eit_info->section0.component_desc[i].component_type = convertStrToInt(component_type);
			str_eit_info->section0.component_desc[i].set_component_tag = atoi(set_component_tag);
			strncpy(str_eit_info->section0.component_desc[i].text,text,COMPONENTDESC_TEXT_LENGTH);
			strncpy(str_eit_info->section0.component_desc[i].lang,lang,COMPONENTDESC_LANG_LENGTH);

			str_eit_info->section1.component_desc[i].stream_content = convertStrToInt(stream_content);
			str_eit_info->section1.component_desc[i].component_type = convertStrToInt(component_type);
	...
	for(i=0;i<CONTENT_DESC_SIZE;i++) {
		...
		char *level1 = getValueFromIni("CONTENT_DESCRIPTOR","level_1",i+1);
		char *level2 = getValueFromIni("CONTENT_DESCRIPTOR","level_2",i+1);
		char *user = getValueFromIni("CONTENT_DESCRIPTOR","user",i+1);

		if ( *level1 != '\0' && *level2  != '\0' && *user != '\0' ) {
			str_eit_info->section0.content_desc[i].level_1 = convertStrToInt(level1);
			str_eit_info->section0.content_desc[i].level_2 = convertStrToInt(level2);
			str_eit_info->section0.content_desc[i].user = convertStrToInt(user);
	...
	for(i=0;i<CA_SYSTEM_ID_SIZE;i++) {
		char *CA_system_id = getValueFromIni("CA_IDENTIFIER_DESCRIPTOR","CA_system_id",i+1);
	...
	int sbLeakRate = convertStrToInt(rechercherValeur("SHORT_SMOOTHING_BUFFER_DESCRIPTOR","sb_leak_rate"));
	str_eit_info->section0.short_smoothing_Buffer_desc.sbLeakRate = sbLeakRate;
	str_eit_info->section1.short_smoothing_Buffer_desc.sbLeakRate = sbLeakRate;
	
	strncpy(str_eit_info->section0.parent_rating_desc.country_code,rechercherValeur("PARENTAL_RATING_DESCRIPTOR","country_code"),PARENTALRATINGDESC_COUNTRY_CODE_LENGTH);
	str_eit_info->section0.parent_rating_desc.age = atoi(rechercherValeur("PARENTAL_RATING_DESCRIPTOR","age"));

	strncpy(str_eit_info->section1.parent_rating_desc.country_code,rechercherValeur("PARENTAL_RATING_DESCRIPTOR","country_code"),PARENTALRATINGDESC_COUNTRY_CODE_LENGTH);
	str_eit_info->section1.parent_rating_desc.age = atoi(rechercherValeur("PARENTAL_RATING_DESCRIPTOR","age"));
}
